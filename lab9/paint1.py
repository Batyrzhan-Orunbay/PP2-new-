    ...
    for event in pg.event.get():  # Оқиғаларды өңдеу
        if event.type == pg.QUIT:  # Егер қолданушы жабу батырмасын басса
            ...
            if drawing:  # Егер сызу режимі қосулы болса
                if mode == "pen":  # Егер қалам режимі болса
                    pg.draw.line(screen, current_color, last_pos, event.pos, 10)  # Соңғы және жаңа нүкте арасында сызық сызу
            ...
            if event.button == 1:  # Сол жақ батырма
                ...
                if mode == "rect": # Егер тікбұрыш режимінде болсақ – тікбұрыш салу
                    rect = pg.Rect(start_pos, (end_pos[0] - start_pos[0], end_pos[1] - start_pos[1]))
                    pg.draw.rect(screen, current_color, rect, 2)

                elif mode == "circle":  # Егер шеңбер режимінде болсақ – шеңбер салу
                ...
                elif mode == "square":  # Егер квадрат режимінде болсақ – квадрат салу
                    side = min(abs(end_pos[0] - start_pos[0]), abs(end_pos[1] - start_pos[1]))  # Квадраттың жағын есептеу
                    rect = pg.Rect(start_pos, (side, side))
                    pg.draw.rect(screen, current_color, rect, 2)

                elif mode == "right triangle":  # Егер дұрыс үшбұрыш режимінде болсақ
                    x1, y1 = start_pos
                    x2, y2 = end_pos
                ...
                elif mode == "equilateral triangle":  # Егер тең қабырғалы үшбұрыш режимінде болсақ
                    x1, y1 = start_pos
                    x2, y2 = end_pos
                ...
                elif mode == "rhombus":  # Егер rhombus режимінде болсақ – ромб салу
                    x1, y1 = start_pos
                    x2, y2 = end_pos
                    width = abs(x2 - x1)  # Ені
